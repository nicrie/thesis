...
rot = xe.cross.HilbertCPCCA.load(root_dir + f"models/{model}_{alpha:.2f}")
...
map_proj = {
    "sst": ccrs.EqualEarth(central_longitude=180),
    "prcp": ccrs.EqualEarth(central_longitude=180),
}
...
P0.real.sel(mode=mode).plot(ax=ax5, alpha=0.5, label="u850")
P1.real.sel(mode=mode).plot(ax=ax5, alpha=0.5, label="u200")
...
ax1.set_title(f"Zonal Winds at 850 hPa \n{fve_x*100:.1f} %", loc="center")
ax2.set_title(f"Zonal Winds at 200 hPa \n{fve_y*100:.1f} %", loc="center")
ax5.set_title("")
ax5.text(
    0.02,
    1,
...
for ax in ax_comps:
    ax.coastlines(lw=0.4, color=".5")
    # set the outline thickness of the map projections
...
gl3 = ax3.gridlines(draw_labels=["left", "bottom"], linewidth=0.3)
...
# Save figure
save_to = get_figure_path("chapter6", "raster", f"mjo_mode{mode:02d}.png")
