...
\begin{figure}[ht]
    ...
    \caption[Survey locations and temporal trends in beach litter monitoring.]{\textbf{Survey locations and temporal trends in beach litter monitoring.} This figure presents an overview of the spatial and temporal distribution of beach litter surveys included in the OSPAR dataset \cite{ospar_comission_convention_1992}. The preprocessed dataset comprises \num{3866} surveys conducted on 168 beaches across the North-East Atlantic (Section~\ref{sec:plastics:methods:ospar}) \textbf{(A)} Temporal trends in survey frequency, illustrating the number of surveys conducted per year (left panel) and the cumulative number of surveys (right panel) from 2001 to 2020, stratified by season. Survey frequency has increased markedly, from approximately 100 surveys per year in the early 2000s to nearly 400 in recent years. The total number of surveys for each season is detailed in the legend. \textbf{(C)} Seasonal distribution of survey counts per beach, where each point represents a beach. The distribution density is visualized by plot width, with horizontal dashed lines marking the median (white dashed line) and interquartile range (white dotted line). Across seasons, surveys are distributed relatively evenly, with medians of three to five surveys per season. Winter has the fewest surveys, while autumn has the most.}
