...
\begin{align}
    k(b,b') &= k_{\text{short}}^{\left[3/2\right]}(b, b') + k_{\text{long}}^{\left[3/2\right]}(b, b'),
\end{align} where 
\begin{align}
    k_i^{\left[3/2\right]}(b, b') &= \eta_i^2 \left(1 +  \frac{\sqrt{3}d}{\ell_i} \right) \exp \left(- \frac{\sqrt{3}d}{\ell_i} \right), \quad i=\{\text{short, long}\}.
\end{align}

Here, $\eta_i^2$ and $\ell_i$ represent the variance and spatial scale parameters of the kernels, and $d$ denotes the chordal distance \cite{jeong_spherical_2017} between locations $b$ and $b'$. The parameters $\ell_i$ define the typical length scales over which spatial autocorrelation occurs, while $\eta_i$ modulate the amplitude of these correlations (see Section~\ref{sec:gp:regression}).
...
For the length scales $\ell_i$, positive-valued Inverse Gamma distributions $\ell_i \sim \mathcal{IG}(\gamma_i, \delta_i)$ were used. The hyperparameters $\gamma_i$ and $\delta_i$ were defined such that the \qty{95}{\percent} HDIs extend from \qtyrange{10}{100}{km} for short length scales ($i=\text{short}$) and from \qtyrange{100}{5000}{km} for medium-to-long length scales ($i=\text{long}$), effectively capturing different spatial autocorrelation ranges.
...
\begin{table}[t]
    \caption[Inference of parameters for the log-Gaussian Cox process model.]{\textbf{Inference of parameters for the log-Gaussian Cox process model.} Estimated parameters include variances ($\eta_i^2$), length scales ($\ell_i$, measured in \unit{km}, with $i \in \{\text{short}, \text{long}\}$), spatial mean ($\mu_{\mu}$), and over-dispersion ($\phi$). Values are reported as the 50\textsuperscript{th} percentile (median) alongside their \qty{95}{\percent} highest density intervals (HDIs), expressed as minimum width Bayesian credible intervals (BCIs, in parentheses).}
    \label{tab:model_parameter_posterior}
    \footnotesize
    \centerline{
        \begin{tabular}{@{}lllllll@{}}
            \toprule
            & $\eta_{\text{short}}^2$               & $\ell_\text{short}$         & $\eta_{\text{long}}^2$              & $\ell_{\text{long}}$            & $\mu_{\mu}$           & $\phi$               \\ \midrule
